...
#define LOG_TANK_FILE 1
#define LOG_TANK_CONSOLE 0
...
#include "pure_log.h"
    ...
    */
    ...
    s32 CLog::set_log_tank(const char *tank)
    {
	if (0 == strcmp("file", tank)) {

	    log_tank = LOG_TANK_FILE;
	}
	else if (0 == strcmp("console", tank))
	{
	    log_tank = LOG_TANK_CONSOLE;
	}
    ...
    s32 CLog::set_log_out_by_time(int h_or_d)
    {
		...
		if(h_or_d >0)
		{
			//printf("cut log file by hour\n");
			m_time_stratege_type = HOUR;
		}
		else
		{
			//printf("cut log file by day\n");
